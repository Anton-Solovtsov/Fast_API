...
def create_task(payload:CategoryCreateSchema, category_service: CategoryService = Depends(get_category_service)):
    return category_service.create_category(category_create=payload)

@router.patch('/{category_id}', status_code=status.HTTP_200_OK)
def update_task(category_id, payload:CategoryUpdateSchema, category_service: CategoryService = Depends(get_category_service)):
    return category_service.update_category(category_id=category_id, cat_update=payload)

@router.delete('/{category_id}', status_code=status.HTTP_204_NO_CONTENT)
def delete_task(category_id, category_service: CategoryService = Depends(get_category_service)):
